...
When the event listener is triggered, React calculates the 'update lane' based on the event's priority, and creates an update object. This object and the corresponding fiber are then passed to `enqueueConcurrentHookUpdate()` to mark the fiber for updating, followed by a call to `scheduleUpdateOnFiber()` to schedule the update .
