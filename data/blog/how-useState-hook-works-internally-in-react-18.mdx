...
- [1. Two Implementations of `useState`](#1-two-implementations-of-usestate)
- [2. State Initialization During Mount Phase](#2-state-initialization-during-mount-phase)
...
- [5. State Updates During the Re-Render Phase](#5-state-updates-during-the-re-render-phase)
...
## [1. Two Implementations of `useState`](#1-two-implementations-of-usestate)
React handles hooks like `useState` through a dispatcher mechanism, which ensures that the correct implementation of a hook is called based on the rendering phase—whether the component is being rendered for the first time (mount) or during subsequent renders (update).\
Each hook in React is implemented in two forms:
- mountFoo: Initializes the hook during the component's initial render.
- updateFoo: Handles updates when the component re-renders in subsequent updates.
