...
draft: false
summary: 'React categorizes tasks into 31 update lanes, each representing different priority levels. React schedules and executes these updates by enqueuing tasks into a task queue that integrates with the JavaScript event loop. It processes tasks in small chunks, yielding control back to the browser every 5ms to keep the UI responsive.'
images: []
---
...
- [Handling Update Priority](#assign-updates-priority)
  ...
  - [3. Batching and Interupting Updates](#3-batching-and-interupting-updates)
- [Task Scheduler](#managing-the-task-queue)
  - [1. React Internal Task Queue](#1-react-internal-task-queue)
  - [2. Integrating with the JavaScript Event Loop](#2-two-distinct-task-queues)
  - [3. Enqueuing Tasks to the JavaScript Task Queue](#3-enqueuing-tasks-to-the-javascript-task-queue)
  - [4. Executing React task](#4-executing-react-task)
...
- **Batching Updates**: When the newly identified highest priority update matches the current pending task (`root.callbackPriority`), React skips scheduling a new task and returns immediately, thus the current scheduled task will process the new update with others together.
...
d) **Scheduling New Tasks**: If the highest priority update lane is `SyncLane`, React recognizes the need for immediate execution. In this case, it invokes `scheduleSyncCallback()` to maintain a queue of synchronous tasks. Following this, `flushSyncCallbacks()` is called to execute all pending updates in the highest priority lane without delay. \
...
Here’s how React interacts with the event loop:

- a). **Enqueuing `performWorkUntilDeadline()` into JavaScript Task Queue**
...
- b). **Executing and Managing Tasks in the JavaScript Event Loop**
...
React can handle even the most intensive rendering tasks without compromising the responsiveness of the user interface.
...
### [4. Executing React task](#4-executing-react-task)
This function is what React places in the JavaScript task queue to manage the execution of its internal tasks. When JavaScript event loop picks up `performWorkUntilDeadline()` and executes it, `performWorkUntilDeadline()` first records the current time as the start time and subsequently invokes `workLoop().
...
![wookloop](/static/images/posts/concurrent-mode-and-update-priority-in-react18/wookloop.png)
...
![workloopcurrent](/static/images/posts/concurrent-mode-and-update-priority-in-react18/workloopcurrent.png)
