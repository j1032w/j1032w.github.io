---
title: 'How useState Works Internally in React 18'
date: '2024-03-14'
lastmod: '2024-03-14'
tags: ['react-js', 'in-depth']
draft: false
summary: 'When you call setState(), React doesn''t immediately update the state. Instead, it creates an update object and places it in a queue within the component''s fiber, which is the underlying data structure that React uses to manage and reconcile the component tree.'
images: []
---
...
- [1. `resolveDispatcher()` Function](#resolveDispatcher)
- [2. `mountState()` Function](#mountState)
- [3. The Interleaved Queue (memoizedState.queue.interleaved)](#interleaved-queue)
- [4. The Pending Queue (memoizedState.queue.pending)](#pending-queue)
- [5. updateState](#updateState)
- [Conclusion](#conclusion)


The call graph is illustrated as below:
![useState-call-graph](/static/images/posts/how-useState-works-internally-in-react-18/useState-call-graph.png)

![fiber-memoizedState-with-pending-updates](/static/images/posts/how-useState-works-internally-in-react-18/fiber-memoizedState-with-pending-updates.png)
...
## [1. `resolveDispatcher()` Function](#resolveDispatcher)
Each hook in React has two implementations:
- mountFoo: Initializes the hook during the component's initial render.
- updateFoo: Manages updates in subsequent renders.
...
```
![renderWithHooks](/static/images/posts/how-useState-works-internally-in-react-18/renderWithHooks.png)
...
```
![fiber-memoizedState-structure](/static/images/posts/how-useState-works-internally-in-react-18/fiber-memoizedState-structure.png)
...
![hook-work-in-progress](/static/images/posts/how-useState-works-internally-in-react-18/hook-work-in-progress.png)
...
![fiber-memoizedState-screenshot](/static/images/posts/how-useState-works-internally-in-react-18/fiber-memoizedState-screenshot.png)
...
![pending-update-queue](/static/images/posts/how-useState-works-internally-in-react-18/pending-update-queue.png)


Before `updateReducer()` is invoked, both the `interleaved` and `baseQueue` are merged into the `pending` queue.


![finishQueueingConcurrentUpdates-screenshot](/static/images/posts/how-useState-works-internally-in-react-18/finishQueueingConcurrentUpdates-screenshot.png)
...
![state-update-during-render](/static/images/posts/how-useState-works-internally-in-react-18/state-update-during-render.png)
...
Otherwise, React creates an update object representing the update information, stores it in the fiber, and invokes `scheduleUpdateOnFiber()` to trigger the rendering process.
...
![updateState-call-graph](/static/images/posts/how-useState-works-internally-in-react-18/updateState-call-graph.png)

![updateReducer-sreenshot](/static/images/posts/how-useState-works-internally-in-react-18/updateReducer-sreenshot.png)
